...
## May 31 Rank-Depth Calibration Addendum

Additional matched ToolBench runs were synced after the original pull:

- Same-model DeepSeek top2/top4:
  `toolbench_rank_topk_matched_top2_top4_deepseek`
- Same-model Gemini top2/top4:
  `toolbench_rank_topk_matched_top2_top4_gemini`
- Live L5 cross-model top2/top4:
  `l5_toolbench_rank_top2_top4_matched_clean_w100`
- Live L5 cross-model top3/top5/top10 clean matched rerun:
  `l5_toolbench_rank_topk_matched_clean_w100`

The compact paper-ready outputs are generated by
`scripts/build_rank_depth_calibration.py`:

- `toolbench_same_model_rank_depth.csv`
- `l5_rank_depth_calibration.csv`
- `l5_rank_depth_pool_curve.csv`
- `figures/fig_l5_rank_depth_calibration.png`

Interpretation:

- Same-model top4 recovers for both DeepSeek and Gemini with zero pooled
  conflicts, so the L5 top4 failure is a cross-model verifier-channel effect.
- L5 top2 is packet-limited: it has zero conflicts but only 4--6 all-pooled
  deduplicated packets.
- L5 top4/top10 are conflict-limited: pooled deduplicated packets increase, but
  payload mismatches appear as cross-model rank-path conflicts accumulate.
- The positive operating points are direction-specific: DeepSeek-to-Gemini is
  strongest at top3, while Gemini-to-DeepSeek is strongest at top5.
