# Data audit for the 0510 AsymAgentMark-TK results
...
- The offline decoder proxy supports the channel-knowledge tradeoff: AgentMark-F
  is stronger with exact probabilities, while AsymAgentMark-TK retains more
...
- Strict single-trajectory RLNC payload recovery is materially lower for
  AsymAgentMark-TK than AgentMark-F, especially on ToolBench.
...
| --- | --- | --- |
| Utility is preserved | `utility_retention.csv`, `utility_main.csv` | Supported. ALFWorld Clean 80.04% vs AsymAgentMark-TK 78.58%; ToolBench Clean 75.65% vs AsymAgentMark-TK 77.06%. |
...
| Exact probabilities favor AgentMark-F | `capacity_l0_l6_proxy.csv` payload-8 L0, plus strict RLNC table | Supported. Proxy: ALFWorld AgentMark-F 0.700 vs AsymAgentMark-TK 0.532. Strict global: AgentMark-F 55.58% vs AsymAgentMark-TK 24.83%. |
| Top-10 rank-only verification favors AsymAgentMark-TK over AgentMark-F | `capacity_l0_l6_proxy.csv` payload-8 L1 | Supported only as an offline proxy. ALFWorld AgentMark-F 0.014 vs AsymAgentMark-TK 0.068; ToolBench AgentMark-F 0.003 vs AsymAgentMark-TK 0.032. |
...
| Strict payload attribution is strong | `rlnc_recovery_by_task.csv`, `strict_rlnc_recovery_global.csv` | Not supported for AsymAgentMark-TK. Global strict recovery is 24.83% overall, 34.85% on ALFWorld, and 1.94% on ToolBench. |
...
3. **Cell means and global rates answer different questions.**
   ...
   - AsymAgentMark-TK: 587/2364 = 24.8308%
...
6. **Current evidence-strength claims should use current values.**
   ...
   - ALFWorld AsymAgentMark-TK top10: -11.850
   - ToolBench AsymAgentMark-TK top10: -6.556
...
6. **Longer ToolBench or aggregation strategy.**
   ToolBench strict recovery is very low: AsymAgentMark-TK is 14/720 = 1.9444%.
