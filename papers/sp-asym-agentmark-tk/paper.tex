...
This ablation is a design diagnostic rather than a winner-take-all tuning result. It supports the weak-asymmetry story by showing where rank information becomes useful and where lower-ranked candidates stop paying for their reconstruction risk; Appendix~\ref{app:rank-stability} gives the corresponding stability rationale. Repeated $C_{\mathrm{nom}}$ values reflect candidate-set saturation: increasing the requested $k$ does not add decoded opportunity when the actual visible candidate set or selected decodable path is unchanged. We therefore compare trends rather than treating third-decimal differences as meaningful.
...
We also run ToolBench calibration experiments in which the embedder and verifier use the same top-$k$ depth. This removes a basic confound: if the two sides use different candidate depths, packet scarcity and top-$k$ truncation are entangled. The same-model rerun checks whether the rank codec has enough packet yield under matched conditions; the cross-model rerank checks whether a different verifier model preserves the rank paths well enough for pooled recovery.

Table~\ref{tab:topk-calibration} summarizes the audit-window calibration. With same-model ranking, all tested settings except DeepSeek $k=2$ recover after pooling all splits and seeds, which shows that the codec itself is not the limiting factor at moderate depths. Cross-model verification is more selective: small $k$ can be packet-limited, while larger $k$ can accumulate conflicting packets as lower-ranked candidates become less stable across models. The successful depths are direction-dependent, with DeepSeek-to-Gemini favoring a shallower rank view and Gemini-to-DeepSeek favoring a slightly deeper one.

\begin{table}[t]
\centering
\caption{ToolBench top-$k$ audit calibration. Same-model columns report pooled recovery. Cross-model columns report pooled recovery / packet conflicts, showing where verifier-rank disagreement dominates.}
\label{tab:topk-calibration}
\resizebox{\columnwidth}{!}{\begin{tabular}{lrrrr}
\toprule
$k$ & DS same & Gemini same & DS$\rightarrow$Gemini & Gemini$\rightarrow$DS \\
\midrule
2 & .00 & 1.00 & .00/0 & .00/0 \\
3 & 1.00 & 1.00 & 1.00/15 & .00/5 \\
4 & 1.00 & 1.00 & .00/14 & .00/17 \\
5 & 1.00 & 1.00 & .00/12 & 1.00/7 \\
10 & 1.00 & 1.00 & .00/22 & .00/11 \\
\bottomrule
\end{tabular}}
\end{table}
