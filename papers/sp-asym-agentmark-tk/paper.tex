...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Effective-capacity and audit-window evidence model.} We adopt $\Ceff=C_{\mathrm{nom}}\times\mathrm{DSR}$ to score capacity that survives the verifier's channel, and separate strict per-trajectory attribution from pooled audit-window recovery through the packet-scarcity bound in Proposition~3.
...
For bit-level triage before strict RLNC recovery, a one-sided binomial test with match probability $q>1/2$ under the watermark and $1/2$ under the null needs on the order of
\begin{equation}
...
We evaluate on ALFWorld~\cite{shridhar2021alfworld} and ToolBench~\cite{qin2023toolllm}. ALFWorld tests embodied planning over in-distribution (ID) and out-of-distribution (OOD) splits; ToolBench tests tool-use behavior across six split categories. The LLM endpoints are DeepSeek Chat (\texttt{deepseek-chat})~\cite{deepseek2024} and Gemini 2.0 Flash (\texttt{gemini-2.0-flash})~\cite{team2023gemini}. The postprocessed corpus contains 11,820 trajectories and 4,728 watermark decode-capable trajectories. Methods are: vanilla LLM agent, red-green (RG) behavior bias, symmetric \agentmark{}, and rank-based \ours{}; the clean AgentMark pipeline without embedding is reported for ALFWorld. RG is a deliberately simple keyed baseline that biases choices toward a pseudorandom green set; it is included to show the utility and distributional cost of watermarking that does not preserve the behavior policy.
...
\subsection{Undetectability Axis: Utility Preservation}

Table~\ref{tab:utility} reports average task success and steps for the watermarking and bias baselines. On ALFWorld, \ours{} reaches 78.6\% success versus 80.0\% for clean and 79.4\% for \agentmark{}. On ToolBench, \ours{} reaches 77.1\%. The source utility artifact records run-level variation: ALFWorld rank groups have standard deviations up to 2.89 percentage points, while ToolBench rank entries are about 11 percentage points because they aggregate six heterogeneous splits. We therefore interpret these aggregates conservatively: rank encoding stays close to the ALFWorld clean and symmetric pipelines and does not exhibit the large utility tax associated with naive behavior bias, but small one- or two-point differences are not treated as significant. Vanilla agent utility is reported separately in Appendix~\ref{app:vanilla-baseline}.
...
\subsection{Undetectability Axis: Behavioral Stealth}

We compare empirical action distributions against clean trajectories using Jensen-Shannon divergence (JSD). On ALFWorld, mean JSD to clean is 0.121 for \ours{} and 0.122 for \agentmark{}. RG is comparable on ALFWorld. These measurements are coarse because behavior distributions are task- and split-dependent, but they show no obvious distributional anomaly introduced by rank splitting.
...
\resizebox{\columnwidth}{!}{\begin{tabular}{lrrr}
\toprule
Dataset & \agentmark{} & \ours{} & RG \\
\midrule
ALFWorld & 0.122 & 0.121 & 0.134 \\
\bottomrule
\end{tabular}}
\end{table}
...
The same mechanism appears more clearly in pooled audit windows. Figure~\ref{fig:rank-only-pooled} compares ALFWorld/DeepSeek/ID top-10 recovery as the verifier pools 1--50 trajectories from the same audit configuration. \agentmark{} reaches perfect recovery by pool size 5 when exact probabilities are available, confirming that the baseline is not packet-limited in its native channel. Under a rank-only surrogate that keeps only the top-10 order and assigns monotone linear pseudo-probabilities before running the \agentmark{} differential decoder, pooling does not rescue the baseline: recovery remains between 0.013 and 0.247 over the plotted window. In contrast, \ours{} rises from 0.080 at one trajectory to 0.613 at 10, 0.780 at 20, and 0.807 at 50. The point is not that \ours{} dominates exact-channel \agentmark{}; it is that the rank-only decoder matches the verifier channel, while a probability-bin decoder continues to accumulate conflicting equations even when the audit window grows.
...
Vanilla agent results are kept separate from the paired ALFWorld/ToolBench comparison tables because they are not watermarking or behavior-bias configurations. In the canonical 0510 utility artifact, the vanilla agent reaches 69.1\% success with 16.54 steps on ALFWorld and 84.1\% success with 4.06 steps on ToolBench.

\begin{table}[h]
\centering
\caption{Vanilla agent utility, reported separately from the main paired comparison tables.}
\label{tab:vanilla-baseline}
\resizebox{\columnwidth}{!}{\begin{tabular}{lrrr}
\toprule
Dataset & Success & Steps & Groups \\
\midrule
ALFWorld & 69.1 & 16.54 & 4 \\
ToolBench & 84.1 & 4.06 & 2 \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{itemize}[leftmargin=*]
  ...
  \item Rank-only pooled contrast and unordered-set sanity check: \path{followup_partial_channel/rank_only_pooled_curve_summary.csv}, \path{followup_partial_channel/unordered_set_keyed_partition_summary.json}.
