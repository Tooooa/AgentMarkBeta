...
The implementation is integrated through an \texttt{AgentWatermarker} SDK wrapper with \texttt{algorithm="rank"} as the default. Internally, \texttt{Dist} stores probability tensors and stable indices, \texttt{RankEncStep} applies the interleaved split, and \texttt{RankDecStep} reconstructs bits from sorted indices. The DRBG is an HMAC-SHA512 reseed construction inspired by Meteor~\cite{kaptchuk2021meteor}. A synchronization counter, \texttt{rt\_sync}, forces both sides to advance exactly $\lceil\log_2 n\rceil$ random draws per decoded top-$k$ step. The top/tail gate is outside this shared schedule because tail selections emit no bits and are treated as erasures by the verifier. This detail is small but critical: without fixed top-$k$ consumption, a zero-bit level would shift all later decoding.
