...
\begin{abstract}
Autonomous large language model (LLM) agents increasingly act through tool calls, subgoal choices, and embodied decisions, so provenance must cover behavior trajectories rather than final text alone. Existing behavioral watermarks can preserve an agent's action distribution, but their symmetric decoders require the verifier to reconstruct the same per-step probabilities used during embedding, an assumption that is brittle under API rounding, model refreshes, and proxy-model audits. We propose \ours, a weakly asymmetric behavioral watermark that embeds with full behavior probabilities while allowing verification from only the top-$k$ rank order of candidate behaviors. The method replaces probability-value decoding with rank-path decoding, preserving the agent's marginal behavior while making the verifier depend on a more stable partial view of the behavior channel. On ALFWorld and ToolBench, \ours{} maintains utility and distributional stealth comparable to the clean AgentMark pipeline, while retaining rank-only provenance signal in settings where exact-probability decoding degrades. These results suggest that behavioral watermarking should treat verifier channel knowledge as a first-class design axis: robust agent provenance need not require exact probability reconstruction, but deployment-grade attribution still requires registered payloads and calibrated audit protocols.
\end{abstract}
