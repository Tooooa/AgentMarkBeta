...
Table~\ref{tab:positioning} summarizes the resulting hierarchy. AgentMark is the symmetric behavioral-watermarking framework, and \agentmark{} is its full-probability instance. \framework{} is the weakly asymmetric counterpart: it keeps the policy-preserving behavior carrier but allows the decoder to depend on a verifier-reconstructible invariant rather than exact probability replay. \ours{} is the rank-order instance of this framework, not a claim that the rank primitive itself is new.
...
3. Set $n_t=\min(k,|\B_t|)$. With probability $1-\sum_{i\leq n_t}P_t(\sigma_t(i))$, sample from the tail and return no bits.\\
...
\begin{table}[t]
\centering
\caption{Framework and instance positioning. \agentmark{} is the full-probability instance of the symmetric AgentMark framework; \ours{} is the rank-order instance of \framework{}.}
\label{tab:positioning}
\resizebox{\columnwidth}{!}{\begin{tabular}{llll}
\toprule
Layer & Symmetric side & Weak-asymmetric side & Verifier channel \\
\midrule
Framework & AgentMark & \framework{} & Exact channel vs. invariant \\
Instance & \agentmark{} & \ours{} & Exact $P_t$ vs. top-$k$ rank $\sigma_t$ \\
...
1. Set $n_t=\min(k,|\sigma_t|)$ and derive the same DRBG sequence $r_0,\ldots,r_{\lceil\log_2 n_t\rceil-1}$.\\
...
Table~\ref{tab:coverage} summarizes the evaluation coverage. Utility and stealth metrics use the available methods, with vanilla reported separately in Appendix~\ref{app:vanilla-baseline}. Decode-capable counts apply to the two watermarking methods because vanilla, clean, and RG trajectories do not carry payload metadata. The two watermarked methods are balanced within each dataset, which avoids comparing rank and symmetric decoders on different trajectory pools.
...
\caption{Effective capacity over the L0--L6 verifier-channel ladder. \agentmark{} has high exact-channel capacity on ALFWorld but loses usable capacity as soon as the verifier only has ranks. \ours{} gives up some exact-channel capacity but degrades more gradually under top-$k$ rank views.}
