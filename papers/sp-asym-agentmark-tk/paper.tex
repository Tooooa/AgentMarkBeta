...
\begin{itemize}[leftmargin=*]
  ...
  \item We evaluate on ALFWorld~\cite{shridhar2021alfworld} and ToolBench~\cite{qin2023toolllm} across DeepSeek and Gemini Flash models, measuring utility, stealth, rank-only decoding, selected-only negative controls, top-$k$ sensitivity, rank noise, erasure, semantic rewrite, and false-positive accounting.
\end{itemize}
...
Table~\ref{tab:noise} reports ranking-noise robustness for \ours{} with DeepSeek and top-10 adjacent-swap perturbations. ALFWorld is robust under moderate ranking noise: the ID split remains at 1.0 pooled recovery through $\epsilon=0.5$, and OOD remains at 1.0 through $\epsilon=0.3$ before dropping at $\epsilon=0.5$. ToolBench is harder and degrades under high perturbation, consistent with its lower packet counts. This is the expected boundary of weak asymmetry: probability-value perturbations are harmless when ranks are preserved, but top-$k$ swaps can corrupt the decoded path.

\begin{table}[t]
\centering
\caption{Pooled recovery for \ours{} under adjacent-swap ranking noise. DeepSeek, top-10. Values are corpus-level pooled decode success, not per-trajectory recovery.}
\label{tab:noise}
...
ALFWorld & OOD & 1.00 & 1.00 & 1.00 & 0.67 \\
ToolBench & G1 category & 0.67 & 0.67 & 0.33 & 0.00 \\
ToolBench & G2 category & 0.33 & 0.33 & 0.33 & 0.00 \\
ToolBench & G3 instruction & 0.67 & 0.67 & 0.67 & 0.33 \\
\bottomrule
\end{tabular}}
\end{table}
...
We also evaluate a verifier-side semantic rewrite on ToolBench. The experiment keeps the action set fixed, rewrites the prompt context at five strengths, re-ranks actions with DeepSeek, and decodes from the rewritten ranks. Step-level rank preservation drops substantially under rewriting, so strict per-task recovery remains low. Pooled subsampling shows that enough trajectories can still recover the watermark: at pool size 100 and top-10, recovery is 0.99 for minimal rewrites, 0.97 for light rewrites, 0.82 for medium rewrites, and 0.98 for heavy rewrites. The non-monotonic heavy result reflects sampling and rewrite-distribution effects, so we interpret the experiment as evidence for pool-size robustness rather than as a monotonic rewrite-strength curve.

\begin{table}[t]
\centering
\caption{ToolBench pooled subsample recovery after semantic rewrite. DeepSeek verifier, top-10.}
\label{tab:rewrite}
\resizebox{\columnwidth}{!}{\begin{tabular}{lrr}
\toprule
Rewrite strength & Pool 30 & Pool 100 \\
\midrule
Original & 1.00 & 1.00 \\
Minimal & 0.75 & 0.99 \\
Light & 0.60 & 0.97 \\
...
The current artifact package includes controlled ranking-noise injection, a fixed erasure/channel-degradation rerun, empirical false-positive trials, evidence-strength diagnostics, and a ToolBench semantic-rewrite study. It does not yet include a full live L5 cross-model Top-$k$ rerank/decode table between DeepSeek and Gemini Flash, nor does the semantic-rewrite study cover ALFWorld. These missing perturbations are important for deployment calibration because they test the verifier's rank-reconstruction pipeline rather than only the probability-versus-rank decoder boundary isolated here. The theory covers cross-model L5 only to the extent that the cross-model verifier preserves top-$k$ rank paths; measuring that preservation remains empirical deployment work.
...
\begin{itemize}[leftmargin=*]
  ...
  \item Semantic rewrite and false positives: the ToolBench five-level rewrite summaries and \path{stage_c_3_4_false_positive_summary.csv}.
