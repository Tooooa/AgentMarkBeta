...
\framework{} is defined by the separation between a policy-preserving embedding channel and a verifier-reconstructible decoding invariant. Top-$k$ rank order is the invariant we implement and evaluate because it is naturally exposed by restricted agent APIs and sufficient to recover the interleaved partition path. Other candidate invariants could instantiate the same framework in future work, including coarse quantized probability buckets, signs of pairwise comparisons or Kendall-style order constraints, and top-1 plus unordered candidate-set information. These alternatives may trade capacity, stability, and verifier cost differently; this paper focuses on the rank instance because it gives a clean first operating point.
