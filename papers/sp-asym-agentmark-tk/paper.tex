...
Rank perturbations isolate the channel mechanism, but a verifier may also face behavior serializations that are semantically close while textually changed. Table~\ref{tab:semantic-rewrite} reports the ToolBench semantic-rewrite rerun for \ours{} under the top-10 verifier view. The rewrite strength changes the surface form substantially: token Jaccard falls from .532 under light rewrites to .271 under heavy rewrites. The exact rank path is therefore not preserved at the step level, and single trajectories almost never recover the 8-bit payload. However, the pooled audit window still recovers under all three strengths because the verifier accumulates enough surviving packets across 183 trajectories.

\begin{table*}[t]
\centering
\caption{Semantic rewrite robustness for \ours{} on ToolBench, DeepSeek, top-10. Similarity and rank metrics are step-weighted over 359 rewritten steps per strength; strict DSR is single-trajectory payload recovery over 183 trajectories; pooled DSR combines all splits in the audit window.}
\label{tab:semantic-rewrite}
\resizebox{\textwidth}{!}{\begin{tabular}{lrrrrrrrr}
\toprule
Rewrite & ROUGE-L & Jaccard & Rank keep & Top-1 match & Top-10 overlap & Strict DSR & Pooled DSR & Packets/conflicts \\
\midrule
Light & .519 & .532 & .306 & .393 & .593 & .011 & 1.000 & 45/14 \\
Medium & .304 & .316 & .309 & .379 & .593 & .011 & 1.000 & 45/13 \\
Heavy & .272 & .271 & .312 & .409 & .594 & .011 & 1.000 & 45/14 \\
\bottomrule
\end{tabular}}
\end{table*}
