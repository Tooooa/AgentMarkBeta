...
Increasing $k$ raises both the amount of available rank information and the risk of rank instability, matching Proposition~4's saturation view. Table~\ref{tab:topk} reports the matched-top-$k$ ToolBench reruns where the embedder and verifier use the same rank depth and RLNC recovery is evaluated on actual generated trajectories. Single trajectories almost never recover the 8-bit payload because ToolBench traces are short and packet yield is sparse. Pooling all ToolBench splits within each seed produces the audit-window behavior: very small $k$ can be packet-limited, while moderate and larger $k$ provide enough independent packets for same-model recovery.

This calibration is a real decoding result, not the offline capacity proxy used for Table~\ref{tab:utility}. We keep the ALFWorld top-$k$ sweep out of the table because the local ALFWorld artifact is an offline replay over logged rank traces rather than a matched-top-$k$ rerun. The corresponding proxy still helps explain candidate-set saturation, but it should not be read as task-level or audit-window payload recovery.

\begin{table}[t]
\centering
\caption{ToolBench matched-top-$k$ real decoding calibration for \ours{}. Strict recovery is single-trajectory exact payload recovery over 360 trajectories. Pooled recovery is mean$\pm$standard deviation over three seed-level all-split audit windows; packets are mean deduplicated RLNC packets in those pooled windows.}
\label{tab:topk}
\resizebox{\columnwidth}{!}{\begin{tabular}{lrrrrrr}
\toprule
$k$ & DS strict & DS pooled & DS pkt & Gemini strict & Gemini pooled & Gemini pkt \\
\midrule
2 & .000 & .00$\pm$.00 & 2.67 & .000 & .00$\pm$.00 & 2.67 \\
3 & .000 & .33$\pm$.58 & 9.67 & .000 & .00$\pm$.00 & 5.00 \\
4 & .003 & .33$\pm$.58 & 8.67 & .022 & .67$\pm$.58 & 9.67 \\
5 & .008 & .67$\pm$.58 & 10.33 & .003 & .33$\pm$.58 & 7.67 \\
10 & .028 & 1.00$\pm$.00 & 14.00 & .011 & 1.00$\pm$.00 & 12.00 \\
\bottomrule
\end{tabular}}
\end{table}
...
Table~\ref{tab:topk-calibration} summarizes the cross-model audit-window calibration. Cross-model verification is more selective than the same-model reruns in Table~\ref{tab:topk}: small $k$ can be packet-limited, while larger $k$ can accumulate conflicting packets as lower-ranked candidates become less stable across models. The successful depths are direction-dependent, with DeepSeek-to-Gemini favoring a shallower rank view and Gemini-to-DeepSeek favoring a slightly deeper one.

\begin{table}[t]
\centering
\caption{ToolBench cross-model top-$k$ audit calibration. Cells report pooled recovery / packet conflicts, showing where verifier-rank disagreement dominates.}
\label{tab:topk-calibration}
\begin{tabular}{lcc}
\toprule
$k$ & DS$\rightarrow$Gemini & Gemini$\rightarrow$DS \\
\midrule
2 & .00/0 & .00/0 \\
3 & 1.00/15 & .00/5 \\
4 & .00/14 & .00/17 \\
