...
\subsection{Semantic Rewrite Robustness}
...
Figure~\ref{fig:l5-rank-depth} summarizes the cross-model audit-window calibration as an operating-point choice. Cross-model verification is not simply improved by increasing $k$: the useful depth depends on the embedder--verifier direction. DeepSeek-to-Gemini recovers with the shallower top-3 view as the audit window grows, while Gemini-to-DeepSeek needs the slightly deeper top-5 view.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_l5_rank_depth_calibration.png}
\caption{Live ToolBench L5 cross-model operating-point calibration. Curves show sampled pooled payload recovery for two rank depths under cross-model verification. DeepSeek-to-Gemini favors top-3, while Gemini-to-DeepSeek favors top-5, showing that the verifier rank depth must be calibrated for the deployed model pair.}
\label{fig:l5-rank-depth}
\end{figure*}

Figure~\ref{fig:l5-rank-depth} makes the mechanism visible without relying on a separate conflict panel. Increasing $k$ can increase packet yield, but it also asks the verifier to reconstruct lower-ranked candidates whose rank paths are less stable across models. In the rank tree, even a one-position displacement can move a selected behavior to a different parity branch and turn a useful RLNC equation into a conflicting one. Thus the relevant quantity is not nominal depth alone; it is the balance between independent packet yield and rank-path consistency, selected for the model pair.
