...
The evaluation package is organized so that each reported number has a concrete source. The paper package contains the manuscript, resolved bibliography, compiled PDF, and build helper. The rank encoder and decoder are implemented in \texttt{watermark\_sampler.py}, and deterministic RLNC is implemented in \texttt{rlnc\_codec.py}. In the \texttt{week2\_postprocess} result directory, \texttt{stage\_b\_2\_1\_utility.csv} supports Table~\ref{tab:utility}, \texttt{stage\_b\_2\_2\_jsd.csv} supports Table~\ref{tab:jsd}, \texttt{stage\_b\_1\_1\_capacity\_proxy.csv} supports Table~\ref{tab:capacity}, \texttt{stage\_b\_1\_2\_topk\_ablation.csv} supports Table~\ref{tab:topk}, \texttt{stage\_c\_3\_1\_rank\_noise.csv} supports Table~\ref{tab:noise}, and \texttt{stage\_c\_3\_2\_erasure\_channel.csv} supports Table~\ref{tab:erasure}. The false-positive and confidence artifacts, \texttt{stage\_c\_3\_4\_false\_positive.csv} and \texttt{stage\_d\_4\_1\_confidence\_curve.csv}, support Section~\ref{sec:false-positive-results}. Per-stage JSON sidecars mirror the table rows, and the summary JSON records aggregate counts used for Table~\ref{tab:coverage}. We treat these artifacts as a reproducibility trail for the reported postprocessed diagnostics; a live deployment study with end-to-end recovery is complementary evidence rather than something implied by the proxy capacity tables.

\subsection{Research Questions}

The evaluation answers seven questions. \textbf{RQ1}: Does rank-based embedding preserve task utility compared with clean and symmetric AgentMark pipelines? \textbf{RQ2}: Does it leave behavior-frequency distributions close to clean trajectories? \textbf{RQ3}: What is the tradeoff between exact-probability capacity, rank-only robustness, and selected-only negative controls? \textbf{RQ4}: How sensitive is the method to the top-$k$ parameter? \textbf{RQ5}: How quickly does decoding degrade when the verifier's rank order is noisy? \textbf{RQ6}: How does recovery change when trajectory steps are missing? \textbf{RQ7}: What false-positive threshold and bit-level evidence strength should a verifier use before making an attribution claim?
...
We inject rank disagreement through the offline decoder proxy with noise rate $\epsilon$, which lowers the per-bit match probability while keeping decoded length fixed. This is weaker than a live cross-model rank-reconstruction experiment but isolates the expected channel boundary. On ALFWorld, \ours{} maintains a bit-match rate of 0.900 at $\epsilon=0.05$ and 0.858 at $\epsilon=0.10$, degrading to 0.688 at $\epsilon=0.30$. ToolBench starts near chance because of lower decoded length and capacity, but its degradation is smooth. The experiment confirms the expected boundary: rank-only decoding is insensitive to probability values but sensitive to rank disagreement.

\begin{table}[t]
\centering
\caption{Rank-noise sensitivity for \ours{} in the offline decoder proxy. Cells show bit-match / payload recovery.}
\label{tab:noise}
...
\begin{table}[t]
\centering
\caption{ALFWorld erasure sensitivity in the offline decoder proxy. Cells show bit-match / payload recovery.}
\label{tab:erasure}
\resizebox{\columnwidth}{!}{\begin{tabular}{llrrrrr}
\toprule
...
The postprocessed false-positive artifact instantiates this calculation across dataset, model, split, and top-$k$ configurations using 1000 random payload trials per clean trajectory. Because the strict event is exact payload equality for a pre-registered claim, we report the analytic per-claim threshold $2^{-L}$ and use the trials only to size the expected number of accidental matches under the tested trajectory and claim counts. This is why the paper reports false positives as an analytic security threshold rather than as a learned empirical classifier. Appendix~\ref{app:artifacts} maps this and the other reported diagnostics to their source files.
...
The confidence-curve artifact illustrates this distinction. For rank decoding groups with enough observed bits, 10 matched bits correspond only to $\log_{10}p$ between $-3.01$ and $-1.97$, while 50 observed bits push the diagnostic tail to between $-15.05$ and $-11.95$. These values quantify channel evidence, not ownership by themselves: they become attribution evidence only when the same audit also satisfies claim binding, rank-quality gating, and strict payload recovery.
...
\textbf{Unreported verifier perturbations.}
The current artifact package does not include live cross-model re-querying, semantic prompt rewriting, or Kendall-tau-constrained alternate rank lists for every step. These perturbations are important for deployment calibration, but they would test the verifier's rank-reconstruction pipeline rather than the probability-versus-rank decoder boundary isolated here. We therefore report rank-noise and erasure proxies as controlled channel diagnostics and leave live cross-model and rewrite studies as follow-up evidence.
...
The reported tables are tied to postprocessed artifacts as follows. Utility uses \texttt{stage\_b\_2\_1\_utility.csv}; behavioral JSD uses \texttt{stage\_b\_2\_2\_jsd.csv}; exact and rank-only capacity proxies use \texttt{stage\_b\_1\_1\_capacity\_proxy.csv}; the top-$k$ sweep uses \texttt{stage\_b\_1\_2\_topk\_ablation.csv}; rank-noise diagnostics use \texttt{stage\_c\_3\_1\_rank\_noise.csv}; step-erasure diagnostics use \texttt{stage\_c\_3\_2\_erasure\_channel.csv}; false-positive calculations use \texttt{stage\_c\_3\_4\_false\_positive.csv}; and confidence curves use \texttt{stage\_d\_4\_1\_confidence\_curve.csv}. JSON sidecars mirror the CSV rows for automated checks.
