...
- ALFWorld + ToolBench 主实验轨迹生成阶段已完成，canonical 干净目录为 `/root/autodl-tmp/output-0510`。
- `/root/autodl-tmp/output-0510` 中只暴露最终审计接受的合规记录；原不合规或缺失记录已由 `/root/autodl-tmp/AgentMark3/AgentMark/output_supplement_0426_fix` 中的补跑结果替换。
- 完整性：ALFWorld `5 方法 x 2 模型 x 3 轮 x (ID 140 + OOD 134) = 8220` 条；ToolBench `5 方法 x 2 模型 x 3 轮 x 6 split x 20 = 3600` 条。
- 勘误：下文早期记录里出现过 `ID 134 + OOD 140`，实际环境可加载 canonical 范围为 `ID=140`、`OOD=134`。
- 已完成的是轨迹生成 + 效用评测汇总阶段；Capacity、Top-k 消融、跨模型解码、排名噪声鲁棒性、JSD/统计检验仍属于后续分析/解码阶段。
...
| **AsymMark-R（Ours）** | 本文方法 | 与 AgentMark-F 共享候选动作生成、RLNC payload 编码等外围流程；核心水印编码端改为基于 Top-k ranking 的 rank-aware / parity-split 编码，解码侧仅依赖排名知识 |
...
- **方法**：Clean / RG / AgentMark-F / AsymMark-R
...
- **AsymMark-R**：在 L0 时名义容量略低于 AgentMark-F，但从 L1 开始 $C_{\text{eff}}$ 保持稳定，在 L2–L5 区间远超 AgentMark-F
...
**目标**：验证 rank-aware 编码不会显著损害任务效用。由于 AsymMark-R 与 AgentMark-F 的核心水印编码端不同，不能假设二者效用天然一致，需要分别与 Clean / Vanilla 比较。

### 实验设计

- **自变量**：方法（Vanilla / Clean / RG / AgentMark-F / AsymMark-R）× 基座模型
- **因变量**：
...
2. 统计 SR 和平均步骤数
3. 分别比较 AgentMark-F 与 AsymMark-R 相对 Clean / Vanilla 的 SR 与平均步骤数变化，并报告二者差异是否在统计误差范围内

### 预期结果

- Vanilla ≈ Clean 作为效用参考上界
- AgentMark-F 与 AsymMark-R 都应尽量接近 Clean，但二者差异需要实测，不预设完全一致
- 若 AsymMark-R 略低于 AgentMark-F，需要报告 rank-aware 编码带来的 utility-capacity tradeoff
...
- **自变量**：方法（Vanilla / Clean / RG / AgentMark-F / AsymMark-R）
...
- **方法**：AgentMark-F + RLNC / AsymMark-R + RLNC
...
- AsymMark-R + RLNC：擦除 + 降级双重条件下仍可恢复 payload
...
- 重度改写下排名大幅变化，但 AsymMark-R 通过 RLNC 仍有部分恢复能力
...
- **方法**：AgentMark-F（L0） / AsymMark-R（不同 $k$）
...
| A4 |  AsymMark-R | 同上 = **2,364 条** | 行为序列、SR、步骤数、候选动作集合、**每步 Top-k 排名**（k=10）、rank partition / parity split 映射、每步 rank-derived symbol / parity bit、RLNC 编码符号、最终被选动作及其 rank、完整prompt以及模型回复、token消耗、时间；可选保存完整 $P_t$ 供 oracle 分析 |
...
flowchart TD
    ...
    A4["A4: AsymMark-R 轨迹<br>(Rank Encoding + Parity Split + Top-k)"] --> E21
